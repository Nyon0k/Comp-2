...
public class TuiterLite<T> {
    ...
    public TuiterLite() {
        this.usuarioByEmail = new HashMap<String, Usuario>();
        this.contByHashtag = new HashMap<String, Integer>();
    }
    ...
    public Tuite tuitarAlgo(Usuario usuario, String texto) throws UsuarioCadastradoException, TamMaxTuiteUltrapassadoException {
        ...
        for (String hashtag : hashtagsDoTuite) {
            if (contByHashtag.containsKey(hashtag)){
                int contador = contByHashtag.get(hashtag);
                contByHashtag.replace(hashtag, contador + 1);
            } else {
                contByHashtag.put(hashtag, 1);
            }
        }
    ...
    public String getHashtagMaisComum() {
        return Collections.max(contByHashtag.entrySet(), Map.Entry.comparingByValue()).getKey();
    }
}
